            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if c != 2:
                        ...
                        y_center = xywh[1] * screen_size - (xywh[3] * screen_size / 2 * 0.75)
                        distance = ((screen_size/2 - x_center)**2 + (screen_size/2 - y_center)**2)**0.5

                        # 保存目标点信息
                        body_centers.append((x_center, y_center, distance))
                    ...
                    # 如果检测到的目标是头
                        ...
                        # 保存目标点信息
                        head_centers.append((x_center, y_center, distance))

        # 找最近的一个目标，身体
        # try:
        #     body_centers = np.array(body_centers)
        #     idx = int(np.where(body_centers[:, 2:] == min(body_centers[:, 2:]))[0])
        #     x, y = body_centers[idx][:2]
        #     cv2.circle(img0, (int(x), int(y)), 3, (255, 0, 0), -1)

        # 找最近的一个目标，头
        try:
            head_centers = np.array(head_centers)
